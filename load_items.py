...
def contract_data(file_name, orgid, contractid, userid):
    now = datetime.date.today()
    #Connect to excel workbook
    WorkBook = file_name

    excel_book = opx.load_workbook('{}'.format(WorkBook))
    #excel_book = opx.load_workbook('/home/FourierProducts/JobVis/static/reports/TemplateeeItems.xlsx')
    ...
    ActStat = cursor.execute("SELECT a.StatusId from ActivityStatuses a left join Activities s on a.StatusId=s.ActivityId where s.Level = 2 and a.IsDefault=1")
